...
2. Generator 函数的定义使用 `function*` 关键字，函数体内部使用 `yield` 表达式来定义不同的内部状态。
...
5. 星号位置：在 Generator 函数的定义中，`function*` 的星号可以紧跟在 `function` 关键字后面，或者与函数名之间。不同位置的星号都是有效的。

## `yield` 表达式

### Generator 函数的运行逻辑

1. 遇到 `yield` 表达式时，函数暂停执行，将紧跟在 `yield` 后面的表达式的值作为返回的对象的 `value` 属性值。

2. 下一次调用 `next` 方法时，函数继续执行，直到遇到下一个 `yield` 表达式。
...
2. 遍历器对象具有 Symbol.iterator 属性：Generator 函数生成的遍历器对象本身也具有 `Symbol.iterator` 属性，该属性指向遍历器对象生成函数。遍历器对象自己也可以被用于遍历，将 Generator 函数赋值给对象的 `Symbol.iterator` 属性，可以使对象具有 Iterator 接口，从而实现遍历操作。
...
错误的传递和捕获：在 Generator 函数体外使用 `throw()` 方法抛出的错误，可以在 Generator 函数体内用 `try...catch` 捕获。同样地，Generator 函数体内使用 `throw` 关键字抛出的错误，也可以被 Generator 函数体外的 `try...catch` 语句捕获。

2. 错误的传递与终止：一旦 Generator 函数体内的错误被捕获，继续执行 Generator 函数将会恢复正常，并执行到下一个 `yield` 表达式。但是，如果错误未被捕获，Generator 函数的执行将终止，后续的 `yield` 表达式将不再执行。
...
这些方法的作用都是让 Generator 函数恢复执行，并且使用不同的语句替换 `yield` 表达式。
