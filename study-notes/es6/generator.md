...
3. 不使用 `yield` 表达式，这时 Generator 函数就变成一个单纯的暂缓执行函数，只有在调用 `next` 方法时函数才会执行
...
1. 函数可以被赋值给对象的 `Symbol.iterator` 属性，从而使得该对象具有 Iterator 接口，可以使用 `for...of` 循环或者拓展运算符 `...` 进行遍历

2. 函数生成的遍历器对象本身也具有 `Symbol.iterator` 属性，该属性指向遍历器对象生成函数，遍历器对象自己也可以被用于遍历，将函数赋值给对象的 `Symbol.iterator` 属性，使对象具有 Iterator 接口，从而实现遍历操作

## `next` 方法的参数

`next` 方法的参数用于将值传递给 Generator 内部上一个 `yield` 表达式

1. `yield` 表达式本身没有返回值，或者总是返回 `undefined`，当在函数中使用 `yield` 时，可以通过调用 `next` 方法并传入参数，将这个参数作为上一个 `yield` 表达式的返回值

2. 参数传递影响执行：通过 `next` 方法的参数，可以在不同的执行阶段向 Generator 函数内部传递不同的值，在每次恢复执行时，通过参数来调整函数的行为，从而影响生成器的执行流程
...
4. 通过在函数中使用 `yield`，可以为任意对象添加遍历器接口，使其可以使用 `for...of` 循环进行遍历
...
-   在 函数体外使用 `throw()` 方法抛出的错误，可以在函数体内用 `try...catch` 捕获。同样函数体内使用 `throw` 关键字抛出的错误，也可以被函数体外的 `try...catch` 语句捕获

-   一旦函数体内的错误被捕获，继续执行函数将会恢复正常，并执行到下一个 `yield` 表达式。但如果错误未被捕获，函数的执行将终止，后续的 `yield` 表达式将不再执行

-   `throw()` 方法被捕获后，函数会附带执行到下一个 `yield` 表达式，此时等同于调用了一次 `next()` 方法
...
1. 在函数内部调用 `return()` 方法会终止当前的遍历，且返回一个包含指定值的对象，其中 `value` 属性即为传入的参数，`done` 属性会变为 `true`，表示遍历结束

2. 如果在 `return()` 方法调用时没有提供参数，则返回值的 `value` 属性为 `undefined`

3. 如果函数内部存在 `try...finally` 代码块，并且正执行 `try` 代码块时调用了 `return()` 方法，那么会立即进入 `finally` 代码块执行。完整执行完 `finally` 代码块后，整个函数会结束
